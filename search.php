    ...
    if ($is_tag === '#') {
        ...
        $posts = make_select_query(
            ...
            FROM posts p 
                ...
                WHERE p.id IN (SELECT post_id FROM posts_hashtags ph JOIN hashtags h ON ph.hash_id = h.id 
                    WHERE hash_name = '$tag_name')
                GROUP BY p.id ORDER BY published_at"
        );
    } else {
        $sql = "SELECT p.*, name, avatar_img AS avatar, type_name AS type, icon_class AS class,
                COUNT(c.id) AS comments, COUNT(l.id) AS likes
            FROM posts p 
                JOIN users us ON p.user_id = us.id
                JOIN types tp ON p.post_type = tp.id 
