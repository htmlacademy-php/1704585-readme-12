...
if ($id) {
    ...
    if ($check_id) {
        ...
        $message_content = "Здравствуйте, " . $to_user['name'] . ". На вас подписался новый пользователь " .
            $user['name'] . ". Вот ссылка на его профиль: ";

        $message = new Swift_Message();
        $message->setSubject("У вас новый подписчик");
        $message->setTo([$to_user['email'] => $to_user['name']]);
        $message->setBody($message_content . '<a href="http://localhost/profile.php?id=' . $user['id'] . '">' .
            $user['name'] .'</a>', 'text/html');
